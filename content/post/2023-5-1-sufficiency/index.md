...
Fisher's Sufficiency Theorem states that a statistic `\(T(\boldsymbol{X})\)` is sufficient for a parameter `\(\theta\)` if and only if the joint probability distribution of the data `\(\boldsymbol{X}\)` can be factored into two components:
...
An implication of the theorem is that when using likelihood-based inference, two sets of data yielding the same value for the sufficient statistic `\(T(X)\)` will always yield the same inferences about `\(\theta\)`. By the factorization criterion, the likelihood's dependence on `\(\theta\)` is only in conjunction with `\(T(X)\)`. As this is the same in both cases, the dependence on θ will be the same as well, leading to identical inferences.

## Proving Sufficiency using Fischer's Sufficiency Theorem
...
## Usefulness of Sufficiency

The Rao-Blackwell Theorem shows the usefulness of sufficiency because it allows us to construct improved estimators by conditioning on the sufficient statistic. In other words, the sufficient statistic contains all the relevant information needed for parameter estimation, and conditioning on it helps to eliminate any unnecessary variability in the original estimator, thereby improving its MSE.
...
The Rao-Blackwell Theorem guarantees that the mean squared error (MSE) of the improved estimator `\(\hat{\sigma}^{2*}(T(\boldsymbol{X}))\)` is smaller than or equal to the MSE of the original estimator `\(\hat{\sigma}^2(\boldsymbol{X})\)`. This demonstrates the usefulness of sufficiency in constructing improved estimators.

In conclusion, the Rao-Blackwell Theorem provides a powerful tool for improving the quality of estimators by conditioning on sufficient statistics. The example with Normal samples illustrates the practical application of the theorem and emphasizes the benefits of using sufficient statistics in estimation tasks.
